...
const LeftSidebar: React.FC<SidebarProps> = ({ avatar, username, account }) => {
  const instance = Instance();
  const navigate = useNavigate();
  const location = useLocation();
  const [showLogout, setShowLogout] = useState(false);
  const [showPostModal, setShowPostModal] = useState(false); // 控制发布帖子弹窗的显示
  ...
  return (
    <div className="h-screen bg-white text-gray-800 flex flex-col relative overflow-y-auto scroll-container">
      ...
      <div className="p-4">
        <button
          onClick={() => setShowPostModal(true)} // 打开弹窗
      ...
      <div className="flex items-center justify-between p-4 cursor-pointer">
        <div
          className={`flex items-center transition-transform duration-500 ${
            showLogout ? "transform rotate-y-180" : ""
          }`}
        >
          <div
          ...
          >
            ...
            >
              <div className="bg-red-500 rounded-xl h-12 w-1/4 flex items-center justify-center absolute left-1 top-[4px] group-hover:w-[210px] z-10 duration-500">
                <svg
